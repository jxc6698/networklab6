...
//	struct tcp_hdr *tcp =(struct tcp_hdr *)( buff + sizeof(struct ip4hdr) );
	...
	src = device[index].ip ;
	ip_pack( buf , 20 , len+20 , 1 , 0 , 60 ,  IPPROTO_RAW , src , dst ) ;
	*addr_src = src ;
	*addr = dst ;
...
void ether_send( _U8* buf , int len , _U32 dst )
{
	...
	int arp_num = search_arp_cache( dst ) ;

	memcpy( buff , arp_table[arp_num].mac_addr , 6 );
	memcpy( buff+6 , device[index].mac_addr , 6 ) ;
